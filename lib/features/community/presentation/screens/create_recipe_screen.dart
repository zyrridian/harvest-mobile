...
class _CreateRecipeScreenState extends ConsumerState<CreateRecipeScreen> {
  ...
  void _removeIngredientRow(int index) {
    setState(() {
      (_ingredients[index]['nameController'] as TextEditingController).dispose();
      (_ingredients[index]['qtyController'] as TextEditingController).dispose();
      (_ingredients[index]['unitController'] as TextEditingController).dispose();
  ...
  Future<void> _submitRecipe() async {
    ...
    final formattedIngredients = _ingredients.map((ing) {
      final qtyText = (ing['qtyController'] as TextEditingController).text;
      return {
        'name': (ing['nameController'] as TextEditingController).text.trim(),
        'quantity': qtyText.isNotEmpty ? double.tryParse(qtyText) : null,
        'unit': (ing['unitController'] as TextEditingController).text.trim(),
      };
    }).where((ing) => (ing['name'] as String).isNotEmpty).toList();
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              maxLines: 3,
              decoration: _inputDecoration('Share a little bit about this recipe...'),
            ),
            const SizedBox(height: 24),
                  ...
                  children: [
                    Expanded(
                      ...
                      child: TextField(
                        ...
                        decoration: _inputDecoration('Ingredient (e.g. Flour)'),
                      ),
                    ),
